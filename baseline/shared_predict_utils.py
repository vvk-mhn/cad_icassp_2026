...
def estimate_vocals(
    signal: np.ndarray, sample_rate: int, model: Module, device: str = "mps"
) -> np.ndarray:
    """
...
def load_ensemble_features(cfg, split: str, systems: list) -> pd.DataFrame:
    """Load dataset records and add prediction scores for multiple systems.

    Args:
        cfg (DictConfig): Configuration object.
        split (str): Dataset split to load ('train' or 'valid')
        systems (list): List of baseline system names (e.g., ["songprep_wer", "whisper"])
    ...
    # Load scores for each system provided in the 'systems' list
    for system_name in systems:
        # Score file is expected in the current directory (exp/)
        # File name: <DATASET>.<SPLIT>.<SYSTEM_NAME>.jsonl
        system_path = f"{cfg.data.dataset}.{split}.{system_name}.jsonl"
        
        try:
            system_score = read_jsonl(str(system_path))
            system_score_index = {
                record["signal"]: record[system_name] for record in system_score
            }
            # Add the score column to the main DataFrame
            df[system_name] = df["signal"].map(system_score_index)
            logger.info(f"Successfully loaded scores for system: {system_name}")
        except FileNotFoundError:
            logger.error(f"Score file not found: {system_path}")
            # Fill with NaN or raise error depending on robustness needed
            raise FileNotFoundError(f"Required score file not found: {system_path}")
        except KeyError:
            logger.error(f"Key '{system_name}' not found in {system_path}")
            raise KeyError(f"Key '{system_name}' not found in {system_path} records.")
